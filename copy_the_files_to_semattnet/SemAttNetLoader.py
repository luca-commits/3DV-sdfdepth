...
class SemAttDataset(torch.utils.data.Dataset):
        ...
        for folder in self.target_folders: 
            ...
            for camera_folder in camera_folders:
                ...
                for image in list(os.scandir(camera_folder)):
                    ...
                    cont_depth_path = os.path.join(cont_depth_dir, scene_date, scene_name, camera_name, "data", image_name)
                    if (not os.path.exists(cont_depth_path)):
                        os.path.mkdir(cont_depth_path)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.BoolTensor]:
        ...
        semantic_path = self.target_path[idx]
        rgb, semantic, sparse, target, position = transforms.val_transform(rgb_read(img_path), semantic_read(semantic_path), depth_read(target_path), None, None, self.args)
