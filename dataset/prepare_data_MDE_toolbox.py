...
renders_dir = "/home/casimir/ETH/kitti/renders_50m_size"
...
for model in tqdm(models):
    ...
    for angle in ["pos_angle", "neg_angle"]:
        full_path = f"{model_path}/{angle}"
        drive_split = model
        drive = drive_split[:-2]
        split = drive_split[-1]
        date = drive[:10]
        ...
        # Copy RGBs
        rgb_dest_full = f"{rgb_dest}/additional/{model}/{angle}/"
        ...
        # Copy depths
        depth_dest_full = f"{depth_dest}/additional/{model}/{angle}/"
        ...
        for rgb_file, depth_file in zip(rgb_files, depth_files):
            files_file.write(f"additional/{model}/{angle}/{rgb_file} additional/{model}/{angle}/{depth_file} 0.0\n")
